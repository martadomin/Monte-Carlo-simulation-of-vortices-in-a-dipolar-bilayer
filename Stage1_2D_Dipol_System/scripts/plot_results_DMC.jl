...
_, dt_l,  E_l,  err_l  = load_dmc("dmc_N$(num_part)_nr0sq$(nr0_sq)_Nw$(num_walkers)_linear_no_branching.txt")
_, dt_lr, E_lr, err_lr = load_dmc("dmc_N$(num_part)_nr0sq$(nr0_sq)_Nw$(num_walkers)_linear.txt")
_, dt_q,  E_q,  err_q  = load_dmc("dmc_N$(num_part)_nr0sq$(nr0_sq)_Nw$(num_walkers)_quadratic_no_branching.txt")
...
# Normalise
E_l   ./= nr0_sq_32;  err_l  ./= nr0_sq_32
E_lr  ./= nr0_sq_32;  err_lr ./= nr0_sq_32
E_q   ./= nr0_sq_32;  err_q  ./= nr0_sq_32
E_qr  ./= nr0_sq_32;  err_qr ./= nr0_sq_32

# Δτ mask
lo, hi = 1e-5, 1e-3
mask(dt) = (dt .>= lo) .& (dt .<= hi)
ml  = mask(dt_l);  mlr = mask(dt_lr)
mq  = mask(dt_q);  mqr = mask(dt_qr)
...
E0_lr, σ_lr = glm_fit(dt_lr[mlr], E_lr[mlr], err_lr[mlr], 2)
...
savefig(p_final, joinpath(@__DIR__, "..", "data", "plots", "DMC",
